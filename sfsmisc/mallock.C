/* $Id: mallock.C,v 1.8 2003/12/15 04:54:26 dm Exp $ */
...
{
  char stack[256 * 1024];
  char *const start = (char *) &mallock_init;
  extern char end[];
  u_int i;

  if (mlock (start, end - start) < 0)
    fprintf (stderr, "mallock_init: mlock text+data failed (%d)\n", errno);

  for (i = sizeof (stack); i-- > 0;)
    stack[i] = 0;
  if (mlock (stack, sizeof (stack)) < 0)
    fprintf (stderr, "mallock_init: mlock stack failed (%d)\n", errno);
}
...
#define REAL_MORECORE (*__morecore)
...
static int mlock_warned;
static char *maxpinned;

static char *
morecore (ptrdiff_t size)
{
  char *ret;
  if (size < 0) {
    ret = (char *) sbrk (0) - size;
    if (ret < maxpinned) {
      if (munlock (ret, maxpinned - ret) < 0)
	fprintf (stderr, "mallock: munlock heap failed (%d)\n", errno);
      ret = maxpinned;
    }
  }

  ret = (char *) REAL_MORECORE (size);
  if (ret == (char *) (MORECORE_FAILURE))
    return ret;

  if (!maxpinned)
    maxpinned = ret;
  if (maxpinned < ret + size) {
    if (mlock (maxpinned, ret + size - maxpinned) >= 0)
      maxpinned = ret + size;
    else if (!mlock_warned) {
      mlock_warned = 1;
      fprintf (stderr, "mallock: mlock heap failed (%d)\n", errno);
    }
  }
