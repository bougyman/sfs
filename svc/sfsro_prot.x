...
struct sfsro_subkey {
  unsigned vers;
  opaque key<SFSRO_MAX_PRIVATE_KEYSIZE>;
};

struct sfsro_ownerstate_hash {
  unsigned id;
  unsigned keysize;
  unsigned current_vers;
  unsigned final_vers;
  opaque current_key<SFSRO_MAX_PRIVATE_KEYSIZE>;
  opaque final_subkey<SFSRO_MAX_PRIVATE_KEYSIZE>;
  opaque window_start<SFSRO_MAX_PRIVATE_KEYSIZE>;	
 };

struct sfsro_ownerstate_rabin {
  unsigned id;
  unsigned current_vers;
  unsigned final_vers;
  opaque current_key<SFSRO_MAX_PRIVATE_KEYSIZE>;
...
union sfsro_ownerstate switch (sfsro_protocoltype type) {
 case SFSRO_SHA1:
   sfsro_ownerstate_hash osh;
 case SFSRO_RABIN:
   sfsro_ownerstate_rabin osr;
};

struct sfsro_window {
  unsigned vers;
  opaque edge<SFSRO_MAX_PRIVATE_KEYSIZE>;
};

struct sfsro_keyupdate {
